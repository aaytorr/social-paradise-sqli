...
$Setting = array("PerPage" => 21);
$Page = isset($_GET['Page']) ? $_GET['Page'] : 1;
$Page = ($Page < 1 || !is_numeric($Page)) ? 1 : $Page;
$Minimum = ($Page - 1) * $Setting["PerPage"];
$getall = mysqli_query($connection, "SELECT * FROM Items");
$all = mysqli_num_rows($getall);
$allusers = mysqli_query($connection, "SELECT * FROM Items WHERE itemDeleted='0' ORDER BY ID DESC");
			...
			<table width='98%'>
				<tr>
					<td width='40%' style='background-color:#ddd; padding:20px;border:1px solid #AAA;'>
						<?php
if (isset($myU->PowerArtist) && $myU->PowerArtist == "true") {
	echo "
							<div id='SeeWrap'>
								<div id='ProfileText'>
									Upload Item
		...
		<td align='center'>
			<center>
				<table>
					<tr>
						<?php
$q = isset($_POST['q']) ? mysqli_real_escape_string($connection, strip_tags($_POST['q'])) : '';
$s = isset($_POST['s']) ? mysqli_real_escape_string($connection, strip_tags($_POST['s'])) : '';
$pricemin = isset($_GET['min']) ? mysqli_real_escape_string($connection, strip_tags($_GET['min'])) : '';
$pricemax = isset($_GET['max']) ? mysqli_real_escape_string($connection, strip_tags($_GET['max'])) : '';
if (!$s && !$pricemin && !$pricemax) {
	$getItems = mysqli_query($connection, "SELECT * FROM Items WHERE itemDeleted='0' ORDER BY ID DESC LIMIT {$Minimum}, " . $Setting["PerPage"]);
} elseif ($s) {
	$getItems = mysqli_query($connection, "SELECT * FROM Items WHERE Name LIKE '%$q%' AND itemDeleted='0' ORDER BY ID DESC");
}
$Sort = isset($_GET['Sort']) ? mysqli_real_escape_string($connection, strip_tags($_GET['Sort'])) : '';
if ($Sort) {
	$getItems = mysqli_query($connection, "SELECT * FROM Items WHERE Name LIKE '%$q%' AND itemDeleted='0' AND Type='$Sort' ORDER BY ID DESC");
}
if ($Sort == 'Limiteds') {
	$getItems = mysqli_query($connection, "SELECT * FROM Items WHERE Name LIKE '%$q%' AND itemDeleted='0' AND saletype='limited' ORDER BY ID DESC");
}
if ($pricemin && $pricemax) {
	if (is_numeric($pricemin) && is_numeric($pricemax)) {
	} else {
		echo "<script>alert('Your minimum and max prices must be numbers only');</script>";
		return false;
	}
	if ($pricemin < $pricemax) {
	} else {
		echo "<script>alert('Your max price must be greater than your minimum');</script>";
		return false;
	}
	if ($pricemin >= 0 && $pricemax >= 0) {
	} else {
		echo "<script>alert('Your minimum and max prices must be greater than or equal to 0');</script>";
		return false;
	}
	$getItems = mysqli_query($connection, "SELECT * FROM Items WHERE itemDeleted='0' AND saletype!='limited' AND Price>=$pricemin AND Price<=$pricemax ORDER BY ID DESC");
}
$counter = 0;
$total_counter = 0;
if (mysqli_num_rows($getItems) < 1) {
	if (!empty($Sort)) {
...
while ($total_counter < $Setting['PerPage']) {
	...
	echo "<td style='font-size:13px;' align='left' width='100' valign='top'></td>";
}
?>
					</tr>
				</table>
