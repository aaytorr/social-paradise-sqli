...
include_once($_SERVER['DOCUMENT_ROOT']."/Header.php");
			...
			<td>
				...
				<table width='100%'>
					<tr>
						...
						<td>
																		<font color='green'><b>Bux: $".number_format($gI->Price)."</b></font> &nbsp; &nbsp; "; if ($gI->sell == "yes") { if ($Status->ItemStatus == 0) { echo "<input type='submit' id='buttonsmall' value='Buy Now' name='Buy'></form>"; } else { echo "<font color='red'>Sorry, we are not allowing purchases at this time. Please try again later.</font>"; } } if ($gI->saletype == "limited") { echo "<font color='red' style='padding-left:15px;'><b>$gI->numberstock/$gI->numbersales remaining</b></font>"; } else { echo "<font color='purple' style='padding-left:15px;'><b>".$gsold." sold</b></font>"; } echo "
						</td>
					</tr>
				</table>
					<table>
						<tr>
							...
							<td valign='top'>
											...
											<br />
											<br />
											<a href='WhoBought.php?ID=$ID'>Who Bought</a>
											";
											}
											echo "
							</td>
	...
	if ($gI->saletype == "limited" && $gI->numberstock == "0") {
						...
						$numSellers = mysql_num_rows($getSales);
		echo "
		<table width='95%'>
			<tr>
				<td width='50%' valign='top'>
					<div id='ProfileText'>
						Private Sellers ($numSellers)
					</div>
					...
					if ($numSellers == 0) {
						...
						echo "Sorry, there are no users selling this limited item at the moment.";
											...
											if ($Purchase && $tG == "0" && $SaleID) {
												...
												else
												{
												 if ($num_row == 1 && $myU->Bux >= $SalePrice && $row->UserID != $myU->ID) {
													...
													mysql_query("INSERT INTO PMs (SenderID, ReceiveID, Title, Body, time, LookMessage) VALUES('0','$row->UserID','$myU->Username just bought one of your limiteds!','$myU->Username just bought $gI->Name for $SalePrice!','".time()."','0')");
													mysql_query("INSERT INTO PurchaseLog (UserID, Item, Action, TypeStore, ItemType, SellerID, Price, time) VALUES('$myU->ID','$gI->Name','purchased_limited','Shop','limited','$gpo->Username','$SalePrice','".time()."')");
													mysql_query("INSERT INTO PurchaseLog (UserID, Item, Action, TypeStore, ItemType, SellerID, Price, time) VALUES('$row->UserID','$gI->Name','sold_limited','Shop','limited','$myU->Username','$SalePrice','".time()."')");
				...
				<td valign='top'>
					<div id='ProfileText'>
						...
						<table width='100%'>
							...
							";
							if ($gI->NumberSold != 0) { echo "
							<tr>
								<td>
									<b>Recommended Purchase Price:</b>
								</td>
								<td>
									";
									
									$newprivateseller = mysql_query("SELECT * FROM Sales WHERE ItemID='$ID' ORDER BY Amount ASC LIMIT 1");
									$privateseller = mysql_fetch_object($newprivateseller);
									
									$SalePrices = $gI->SalePrices/$gI->NumberSold;
									$Average = round($SalePrices) + $privateseller->Amount / 2;
									echo "<font color='green'>$<b>".round($Average)."</b></font>";
									echo "
								</td>
							</tr>
							"; } echo "
			...
			<td width='60%' valign='top' >
				...
				if ($User) {
					...
					<form action='#comments' method='POST'>
						<textarea name='Reply' rows='5' style='width:90%;' placeholder=''></textarea>
						<br />
				...
				echo "<div style='max-height:400px;overflow-y:auto;height:400px;'>
											...
											<div style='width: ".$width."px; height: ".$height."px; z-index: 3;  background-image: url(Dir/" . $gP->Top . ");'>
												<div style='width: ".$width."px; height: ".$height."px; z-index: 3;  background-image: url(Dir/" . $gP->Hat . ");'>
													<div style='width: ".$width."px; height: ".$height."px; z-index: 3;  background-image: url(Dir/" . $gP->Shoes . ");'>
														...
														echo "<div style='width: ".$width."px; height: ".$height."px; z-index: 3;  background-image: url(/Images/OnlineWatermark.png);'>";
														}
														if (time() < $gO->expireTime) {
														echo "</div>";
														}
					...
					if ($gI->saletype == "limited"&&$gI->numberstock == "0") {
						...
						elseif ($tG == "0") {
							...
							mysql_query("INSERT INTO Logs (UserID, Message, Page) VALUES('".$myU->ID."','purchased ".$gI->Name." for ".$Price."','".$_SERVER['PHP_SELF']."')");
																				...
																				mysql_query("INSERT INTO PurchaseLog (UserID, Item, Action, TypeStore, ItemType, SellerID, Price) VALUES('$myU->ID','$gI->Name','store_purchase','Shop','regular','$myU->Username','$Price')");
